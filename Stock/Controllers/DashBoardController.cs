...
namespace Stock.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        public IActionResult DashboardAdministrativo(int? filtro)
        {
            if (!filtro.HasValue)
            {
                // Si no hay filtro, simplemente mostrar la vista vacía o el stock completo
                filtro = null;
            }
        ...
        [HttpPost]
        public IActionResult EnviarPedidos(List<PedidoRequest> OrdenDeCompra)
        {
            try
            {
                string? cs = _configuration.GetConnectionString("MiConexion");
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    foreach (var p in OrdenDeCompra)
                    {
